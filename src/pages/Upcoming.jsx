...
function Upcoming() {
    ...
    const handlePageChange = (newPage) => {
        if (newPage !== page && newPage >= 1 && newPage <= totalPage && !loadingMore) {
            ...
            <div className='w-full bg-[#10141e] md:p-10 mb-20 md:mb-0'>
                ...
                {!loader && upcoming.length > 0 && (
                    <div className="flex flex-col items-center mt-8 space-y-4">
                        ...
                        <div className="text-gray-400 text-sm text-center flex items-center space-x-2">
                            ...
                            <span>Page {page} of {totalPage} • {upcoming.length} upcoming movies loaded</span>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 flex-wrap justify-center">
                            <button
                                ...
                                className="px-3 py-2 bg-gray-700 text-white rounded hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                                First
                            </button>
                            ...
                            <div className="flex space-x-1">
                                {Array.from({ length: Math.min(5, totalPage) }, (_, i) => {
                                    const pageNum = Math.max(1, Math.min(totalPage - 4, page - 2)) + i;
                        ...
                        <div className="flex items-center space-x-2 bg-green-900/30 px-4 py-2 rounded-full border border-green-600/30">
                            <svg className="w-5 h-5 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-5 5v-5zM4.343 12.344l1.414 1.414L9.5 9.5M12 6V4m6.364 1.636l-1.414 1.414M20 12h-2M6.364 6.364L4.95 4.95M4 12H2m6.364 6.364l1.414-1.414M12 20v-2" />
                            </svg>
                            <span className="text-green-400 text-sm font-medium">Coming Soon to Theaters</span>
                        </div>
                    </div>
                )}
                ...
                {!loader && upcoming.length === 0 && (
                    <div className="text-center py-10">
                        <div className="text-gray-400 text-lg mb-2">No upcoming movies found</div>
                        <div className="text-gray-500 text-sm">Check back later for the latest upcoming releases</div>
                    </div>
                )}
            </div>
        </>
